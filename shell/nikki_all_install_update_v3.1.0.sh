...
#
# 非交互示例：
#   sh nikki_all_install_update_v3.1.0.sh --action smart --lgbm auto --yes
#   sh nikki_all_install_update_v3.1.0.sh --action alpha --yes
#   sh nikki_all_install_update_v3.1.0.sh --action stable --yes
#   sh nikki_all_install_update_v3.1.0.sh --action uninstall --yes
...
usage() {
	cat <<'EOF'
用法：nikki_all_install_update_v3.1.0.sh [选项]
...
install_or_update_nikki() {
	...
	io_success=0
	progress_line "2/4" "检查官方软件源并执行方案 A；失败时自动切换方案 B"
	if feed_present && feed_current; then
...
run_maintenance() {
	...
	if [ "$rm_kind" = skip ] && [ "$MODEL_MAINTAIN" -eq 0 ] && [ "$GEOX_CHOICE" = skip ] && [ "$ZASH_CHOICE" = skip ]; then
		flow_title "步骤 2/5：Mihomo 内核"
		...
		flow_title "步骤 3/5：LightGBM 模型"
		progress_skip "已按计划跳过 LightGBM，保留当前文件"
		flow_title "步骤 4/5：GeoX 数据库"
		progress_skip "已按计划跳过 GeoX，保留当前数据库"
		flow_title "步骤 5/5：Zashboard 面板"
	...
	progress_line "2/2" "备份内核、模型、GeoX、面板和 Nikki 配置"
	...
	case "$rm_kind" in
		...
		alpha)
			rm_old_kind="$CORE_KIND_NEW"; rm_old_core_version="$NEW_CORE_VERSION"
			if prepare_core alpha; then CORE_UPDATE_STATUS="updated"; MAINT_SUCCESS=$((MAINT_SUCCESS + 1));
		...
		stable)
			rm_old_kind="$CORE_KIND_NEW"; rm_old_core_version="$NEW_CORE_VERSION"
			if prepare_core stable; then CORE_UPDATE_STATUS="updated"; MAINT_SUCCESS=$((MAINT_SUCCESS + 1));
...
print_summary() {
	...
	case "$ps_kind" in
		smart)
			...
			;;
		alpha) summary_item "当前开发预览版内核（Prerelease-Alpha）" "$ps_core" "$ps_core_result"; [ "$MODEL_MAINTAIN" -ne 1 ] || summary_item "现有 LightGBM模型" "${ps_model:-unknown}" "$ps_model_result" ;;
		stable) summary_item "当前稳定版内核（meta）" "$ps_core" "$ps_core_result"; [ "$MODEL_MAINTAIN" -ne 1 ] || summary_item "现有 LightGBM模型" "${ps_model:-unknown}" "$ps_model_result" ;;
...
manual_batch_render() {
	...
	manual_option "$MB_NIKKI" "1）安装、更新或修复 Nikki 插件主体"
	say ""
	...
	say "${B}${Y}     以上内核可全部不选：若同时跳过 Nikki 则保留原内核；若安装/更新 Nikki，则使用其默认稳定版内核 mihomo-meta。${N}"
	say ""
	say "${B}${Y}$(manual_model_status)；当前内核为 Smart 或选择 2）Smart 内核时可选${N}"
	manual_option "$MB_MODEL_AUTO" "5）自动选择合适的 LightGBM 模型" "${B}${Y}（根据路由器性能和剩余存储空间自动计算）${N}"
	...
	manual_option "$MB_GEOX" "9）更新整套 GeoX 数据库"
	say ""
	if zashboard_is_installed; then
	...
	say ""
	say "${B}${Y}返回主菜单${N}"
	manual_option "$MB_RETURN" "0）一键返回主菜单（必须单独选择）"
	menu_line "======================================================"
...
automatic_maintenance_menu() {
	...
	while :; do
		say ""
		menu_line "================ 自动维护项目菜单 ================"
		menu_line "  1）按现有组件类型一键更新（必须已安装 Nikki 和内核）"
		say "${B}${Y}     更新 Nikki、现有类型内核，以及已经存在的 LightGBM、GeoX、Zashboard；不会安装当前没有的可选组件。${N}"
		menu_line "  2）一键安装或更新 Nikki + Smart 内核 + 自动 LGBM + GeoX + Zashboard 完整版（dist.zip）"
		say "${B}${Y}     LGBM 将根据系统性能和剩余存储空间自动选择大、中、小模型。${N}"
		menu_line "  3）一键安装或更新 Nikki + Dev 预览版内核 + GeoX + Zashboard 完整版（dist.zip）"
		menu_line "  4）一键安装或更新 Nikki + 稳定版内核 + GeoX + Zashboard 完整版（dist.zip）"
		menu_line "  5）直接返回主菜单"
		menu_line "=================================================="
		prompt '>>> 请手动选择 [1-5]：'
...
run_update_workflow() {
	...
	print_execution_plan
	flow_title "步骤 1/5：Nikki 插件主体及依赖"
...
main() {
	...
	while :; do
		...
		if [ "$MAIN_CHOICE" = auto ]; then
			...
			if [ "$AUTO_CHOICE" = existing ]; then
				configure_existing_auto_plan || { warn "无法建立现有组件更新计划，返回自动维护菜单"; continue; }
			else
				ACTION="$AUTO_CHOICE"
				...
				GEOX_CHOICE=update
				ZASH_CHOICE=update
				set_zashboard_variant dist.zip
				if [ "$ACTION" = smart ]; then LGBM_CHOICE=auto; LGBM_SET=1; MODEL_MAINTAIN=1
				else LGBM_CHOICE=skip; LGBM_SET=1; MODEL_MAINTAIN=0; fi
			fi
