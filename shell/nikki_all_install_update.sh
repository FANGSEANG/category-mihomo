...
# 功能：安装/更新 Nikki、切换 Mihomo 内核、更新 LightGBM/GeoX/Zashboard/已有订阅与 rule-set、卸载清理
#
# 可选环境变量：
...
#
# 非交互示例：
#   sh nikki_all_install_update_v3.5.0.sh --action smart --lgbm auto --yes
#   sh nikki_all_install_update_v3.5.0.sh --action alpha --yes
#   sh nikki_all_install_update_v3.5.0.sh --action stable --yes
#   sh nikki_all_install_update_v3.5.0.sh --action uninstall --yes

set -u

SCRIPT_VERSION="3.5.0"
ACTION=""; CLI_ACTION=""; MAIN_CHOICE=""; WORKFLOW_MODE="命令行维护"
ENVIRONMENT_READY=0; PKG_INDEX_READY=0; NIKKI_FEED_READY=0; NIKKI_FEED_ATTEMPTED=0
...
SUBSCRIPTION_CHOICE="skip"; RULESET_CHOICE="skip"; MODEL_MAINTAIN=0
...
SUBSCRIPTION_UPDATE_STATUS=not_selected; PROXY_PROVIDER_UPDATE_STATUS=not_selected; RULESET_UPDATE_STATUS=not_selected
SUBSCRIPTION_UPDATED_COUNT=0; SUBSCRIPTION_TOTAL_COUNT=0
PROXY_PROVIDER_UPDATED_COUNT=0; PROXY_PROVIDER_TOTAL_COUNT=0; RULESET_UPDATED_COUNT=0; RULESET_TOTAL_COUNT=0
...
inspect_nikki_release_fallback() {
	...
	fetch_url "https://api.github.com/repos/${NIKKI_REPO}/releases/latest" "$inrf_json" json || return 1
...
# Nikki 官方订阅缓存：只更新已经存在的 UCI subscription section；不会创建订阅。
nikki_subscription_sections() {
	command -v uci >/dev/null 2>&1 || return 0
	# -X 禁止把匿名 section 改写成 @subscription[n]，保留 init 脚本可直接识别的 cfgXXXXXX ID。
	uci -q -X show nikki 2>/dev/null | sed -n 's/^nikki\.\([^=]*\)=subscription$/\1/p'
}

nikki_subscription_count() {
	nsc_count=0
	for nsc_section in $(nikki_subscription_sections); do nsc_count=$((nsc_count + 1)); done
	printf '%s' "$nsc_count"
}
...
proxy_provider_names() {
	[ -s "$RUN_DIR/config.yaml" ] || return 0
	command -v yq >/dev/null 2>&1 || return 0
	yq -M -r '.proxy-providers // {} | to_entries | .[] | select((.value.type // "http") == "http") | .key' "$RUN_DIR/config.yaml" 2>/dev/null || true
}

proxy_provider_count() {
	ppc_count=0
	while IFS= read -r ppc_name; do [ -z "$ppc_name" ] || ppc_count=$((ppc_count + 1)); done <<EOF
$(proxy_provider_names)
EOF
	printf '%s' "$ppc_count"
}
...
update_existing_subscriptions() {
	SUBSCRIPTION_TOTAL_COUNT="$(nikki_subscription_count)"
	SUBSCRIPTION_UPDATED_COUNT=0
	if [ "$SUBSCRIPTION_TOTAL_COUNT" -eq 0 ]; then
		SUBSCRIPTION_UPDATE_STATUS="not_present"
		progress_skip "未配置 Nikki 机场订阅，不创建新订阅"
		return 0
	fi
	ues_failed=0; ues_index=0
	for ues_section in $(nikki_subscription_sections); do
		ues_index=$((ues_index + 1))
		ues_name="$(uci -q get "nikki.${ues_section}.name" 2>/dev/null || true)"
		[ -n "$ues_name" ] || ues_name="$ues_section"
		progress_line "${ues_index}/${SUBSCRIPTION_TOTAL_COUNT}" "更新机场订阅：$ues_name"
		if nikki_update_subscription_section "$ues_section" && [ "$(uci -q get "nikki.${ues_section}.success" 2>/dev/null || true)" = 1 ]; then
			SUBSCRIPTION_UPDATED_COUNT=$((SUBSCRIPTION_UPDATED_COUNT + 1))
			progress_done "机场订阅已更新：$ues_name"
		else
			ues_failed=$((ues_failed + 1))
			warn "机场订阅更新失败，官方更新器已保留原缓存：$ues_name"
		fi
	done
	if [ "$SUBSCRIPTION_UPDATED_COUNT" -eq "$SUBSCRIPTION_TOTAL_COUNT" ]; then SUBSCRIPTION_UPDATE_STATUS="updated"; return 0; fi
	if [ "$SUBSCRIPTION_UPDATED_COUNT" -gt 0 ]; then SUBSCRIPTION_UPDATE_STATUS="partial"; return 0; fi
	SUBSCRIPTION_UPDATE_STATUS="skipped"
	return 1
}

nikki_update_subscription_section() {
	"$NIKKI_INIT" update_subscription "$1"
}
...
update_existing_proxy_providers() {
	PROXY_PROVIDER_TOTAL_COUNT="$(proxy_provider_count)"
	PROXY_PROVIDER_UPDATED_COUNT=0
	if [ "$PROXY_PROVIDER_TOTAL_COUNT" -eq 0 ]; then
		PROXY_PROVIDER_UPDATE_STATUS="not_present"
		progress_skip "当前 YAML 没有 HTTP proxy-provider，不创建新机场订阅"
		return 0
	fi
	if ! nikki_running; then
		PROXY_PROVIDER_UPDATE_STATUS="skipped"
		warn "Nikki 当前未运行，无法刷新 YAML proxy-provider；已保留现有缓存"
		return 1
	fi
	if ! mihomo_api_prepare; then
		PROXY_PROVIDER_UPDATE_STATUS="skipped"
		warn "无法读取或连接 Mihomo external-controller，不能刷新 YAML proxy-provider"
		return 1
	fi
	uep_index=0
	while IFS= read -r uep_name; do
		[ -n "$uep_name" ] || continue
		uep_index=$((uep_index + 1))
		progress_line "${uep_index}/${PROXY_PROVIDER_TOTAL_COUNT}" "刷新机场 proxy-provider：$uep_name"
		uep_encoded="$(percent_encode_path_segment "$uep_name" 2>/dev/null || true)"
		if [ -z "$uep_encoded" ]; then warn "proxy-provider 名称编码失败，已跳过：$uep_name"; continue; fi
		if mihomo_api_put "/providers/proxies/${uep_encoded}" >/dev/null 2>&1; then
			PROXY_PROVIDER_UPDATED_COUNT=$((PROXY_PROVIDER_UPDATED_COUNT + 1))
			progress_done "机场 proxy-provider 已刷新：$uep_name"
		else
			warn "机场 proxy-provider 刷新失败，已保留现有缓存：$uep_name"
		fi
	done <<EOF
$(proxy_provider_names)
EOF
	if [ "$PROXY_PROVIDER_UPDATED_COUNT" -eq "$PROXY_PROVIDER_TOTAL_COUNT" ]; then PROXY_PROVIDER_UPDATE_STATUS="updated"; return 0; fi
	if [ "$PROXY_PROVIDER_UPDATED_COUNT" -gt 0 ]; then PROXY_PROVIDER_UPDATE_STATUS="partial"; return 0; fi
	PROXY_PROVIDER_UPDATE_STATUS="skipped"
	return 1
}
...
run_maintenance() {
	...
	if [ "$rm_kind" = skip ] && [ "$MODEL_MAINTAIN" -eq 0 ] && [ "$GEOX_CHOICE" = skip ] && [ "$ZASH_CHOICE" = skip ] && [ "$SUBSCRIPTION_CHOICE" = skip ] && [ "$RULESET_CHOICE" = skip ]; then
		...
		flow_title "步骤 6/6：YAML 配置关联资源"
		progress_skip "已按计划跳过机场订阅和 rule-set 更新"
		...
		SUBSCRIPTION_UPDATE_STATUS="user_skipped"
		PROXY_PROVIDER_UPDATE_STATUS="user_skipped"
	...
	flow_title "步骤 6/6：YAML 配置关联资源"
	say "${B}${Y}  [1/3] Nikki 机场订阅：仅更新已经配置的 UCI 订阅，不创建缺失项目${N}"
	if [ "$SUBSCRIPTION_CHOICE" = update ]; then
		if update_existing_subscriptions; then
			case "$SUBSCRIPTION_UPDATE_STATUS" in updated|partial) MAINT_SUCCESS=$((MAINT_SUCCESS + 1)) ;; esac
		else warn "无法更新机场订阅，已跳过此项；建议稍后在 Nikki 订阅页面手动更新"; fi
	else SUBSCRIPTION_UPDATE_STATUS="user_skipped"; progress_skip "已按计划跳过机场订阅更新"; fi

	# 先提交会影响运行配置的文件和订阅更新，让随后刷新的 provider 对应最新 YAML。
	...
	say "${B}${Y}  [2/3] 机场 proxy-provider：仅刷新当前 YAML 已存在的 HTTP 节点订阅${N}"
	if [ "$SUBSCRIPTION_CHOICE" = update ]; then
		if update_existing_proxy_providers; then
			case "$PROXY_PROVIDER_UPDATE_STATUS" in updated|partial) MAINT_SUCCESS=$((MAINT_SUCCESS + 1)) ;; esac
		else warn "无法刷新 YAML 机场 proxy-provider，已跳过此项；建议稍后在面板中手动更新 provider"; fi
	else PROXY_PROVIDER_UPDATE_STATUS="user_skipped"; progress_skip "已按计划跳过 YAML proxy-provider 更新"; fi

	say "${B}${Y}  [3/3] rule-set：仅刷新当前 YAML 中已经存在的 HTTP rule-provider${N}"
...
print_summary() {
	[ "$SUBSCRIPTION_TOTAL_COUNT" -gt 0 ] || SUBSCRIPTION_TOTAL_COUNT="$(nikki_subscription_count)"
	[ "$PROXY_PROVIDER_TOTAL_COUNT" -gt 0 ] || PROXY_PROVIDER_TOTAL_COUNT="$(proxy_provider_count)"
	...
	ps_subscription_result="$(summary_status "$SUBSCRIPTION_UPDATE_STATUS" "$SUBSCRIPTION_UPDATED_COUNT" "$SUBSCRIPTION_TOTAL_COUNT")"
	ps_proxy_provider_result="$(summary_status "$PROXY_PROVIDER_UPDATE_STATUS" "$PROXY_PROVIDER_UPDATED_COUNT" "$PROXY_PROVIDER_TOTAL_COUNT")"
	...
	say ""
	summary_item "Nikki 机场订阅" "已配置 ${SUBSCRIPTION_TOTAL_COUNT} 项" "$ps_subscription_result"
	summary_item "YAML 机场 proxy-provider" "已配置 ${PROXY_PROVIDER_TOTAL_COUNT} 项 HTTP provider" "$ps_proxy_provider_result"
...
configure_existing_auto_plan() {
	...
	NIKKI_UPDATE_CHOICE=update; SUBSCRIPTION_CHOICE=update; RULESET_CHOICE=update
	...
	info "机场订阅和 rule-set 仅在已有配置时更新；未配置项目会自动跳过"
}
...
configure_full_auto_plan() {
	...
	SUBSCRIPTION_CHOICE=update; RULESET_CHOICE=update
...
configure_core_switch_plan() {
	...
	SUBSCRIPTION_CHOICE=skip; RULESET_CHOICE=skip
	LGBM_SET=1
	if [ "$ACTION" = smart ]; then
		LGBM_CHOICE=auto
		MODEL_MAINTAIN=1
...
configure_component_only_plan() {
	...
	case "$COMPONENT_ONLY_KIND" in nikki|geox|zashboard|subscription|ruleset) ;; *) return 1 ;; esac
	...
	case "$COMPONENT_ONLY_KIND" in
		...
		subscription) SUBSCRIPTION_CHOICE=update ;;
...
manual_status_scan() {
	...
	MS_SUBSCRIPTION_COUNT="$(nikki_subscription_count)"
	MS_PROXY_PROVIDER_COUNT="$(proxy_provider_count)"
	MS_RULESET_COUNT="$(ruleset_provider_count)"
	ok "手动维护状态查询完成；查询失败的项目仍可选择，执行时会重新尝试"
}
...
manual_batch_render() {
	...
	say ""
	say "${B}${Y}YAML 关联资源：只更新已配置项目${N}"
	if [ "${MS_SUBSCRIPTION_COUNT:-0}" -gt 0 ] || [ "${MS_PROXY_PROVIDER_COUNT:-0}" -gt 0 ]; then
		say "  机场订阅：Nikki UCI 已配置 $(paint_current "${MS_SUBSCRIPTION_COUNT:-0}") 项，YAML HTTP proxy-provider $(paint_current "${MS_PROXY_PROVIDER_COUNT:-0}") 项"
	else
		say "  机场订阅：$(paint_missing '未配置') Nikki 订阅或 YAML HTTP proxy-provider；选择后也不会创建"
	fi
	manual_option "$MB_SUBSCRIPTION" "17）更新机场订阅与 proxy-provider（如有）"
	...
	manual_option "$MB_RULESET" "18）更新 rule-set 规则集（如有）"
	say "${B}${Y}     未配置的订阅或 provider 不会创建，直接跳过。${N}"
	say ""
	say "${B}${Y}返回${N}"
	manual_option "$MB_RETURN" "0）返回主菜单（必须单独选择）"
	menu_line "======================================================"
	say "${B}${Y}输入多个编号时用空格或英文逗号分隔，例如：1 2 5 9 10 17 18${N}"
...
manual_batch_reset() {
	...
	MB_SUBSCRIPTION=0; MB_RULESET=0; MB_RETURN=0
}
...
manual_batch_parse() {
	...
	for mbp_code in $mbp_input; do
		case "$mbp_code" in
			...
			17) MB_SUBSCRIPTION=1 ;; 18) MB_RULESET=1 ;;
	...
	mbp_action_count=$((MB_NIKKI + mbp_core_count + mbp_model_count + MB_GEOX + mbp_zash_count + MB_SUBSCRIPTION + MB_RULESET))
...
manual_batch_print_selection() {
	...
	if [ "$MB_SUBSCRIPTION" -eq 1 ]; then menu_line "  [已选] YAML：更新现有机场订阅"; else say "  - YAML/机场订阅：跳过"; fi
...
manual_batch_apply() {
	...
	if [ "$MB_SUBSCRIPTION" -eq 1 ]; then SUBSCRIPTION_CHOICE=update; else SUBSCRIPTION_CHOICE=skip; fi
...
manual_batch_menu() {
	...
	manual_batch_reset
	manual_status_scan "$MB_INSTALLED"
	while :; do
...
main_menu() {
	...
	while :; do
		...
		if [ "$mm_installed" -eq 1 ]; then
			...
			say "${B}${Y}Nikki：${N}$(paint_missing '未安装')"
		fi
...
automatic_maintenance_menu() {
	...
	while :; do
		...
		say "${B}${Y}     更新 Nikki、当前类型内核及已安装的模型、GeoX、面板和 YAML 资源；缺少的可选组件不新装。${N}"
		...
		say "${B}${Y}     2-4 支持首次安装或切换内核；已有机场订阅和 rule-set 同步更新，未配置则跳过。${N}"
		say ""
		say "${B}${M}---------------- 单项维护 ----------------${N}"
		...
		menu_line " 11）机场订阅｜更新 Nikki 订阅与 proxy-provider（如有）"
		menu_line " 12）规则集｜更新 YAML rule-set provider（如有）"
		menu_line " 13）返回主菜单"
		menu_line "=================================================="
		prompt '>>> 请手动选择 [1-13]：'
		...
		case "$USER_INPUT" in
			...
			11) component_only_auto_eligible "$amm_installed" "机场订阅" || continue; AUTO_CHOICE="only-subscription"; return 0 ;;
			12) component_only_auto_eligible "$amm_installed" "rule-set" || continue; AUTO_CHOICE="only-ruleset"; return 0 ;;
...
confirm_uninstall() {
	...
	while :; do
		...
		cu_answer="$(printf '%s' "$USER_INPUT" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' | tr '[:lower:]' '[:upper:]')"
		case "$cu_answer" in
			YES|Y|是) return 0 ;;
			0) return 1 ;;
...
choose_uninstall_feed_policy() {
	...
	while :; do
		...
		cufp_answer="$(printf '%s' "$USER_INPUT" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' | tr '[:lower:]' '[:upper:]')"
		case "$cufp_answer" in
			YES|Y|是) UNINSTALL_REMOVE_FEED=1; return 0 ;;
			NO|N|否) UNINSTALL_REMOVE_FEED=0; return 0 ;;
			0) return 1 ;;
...
set_all_update_statuses() {
	...
	SUBSCRIPTION_UPDATE_STATUS="$saus_value"; PROXY_PROVIDER_UPDATE_STATUS="$saus_value"; RULESET_UPDATE_STATUS="$saus_value"
}
...
print_execution_plan() {
	...
	if [ "$SUBSCRIPTION_CHOICE" = update ]; then say "  ${G}✔${N} 机场订阅：更新已有 Nikki 订阅和 YAML proxy-provider（不存在则跳过）"; else say "  ${Y}－${N} 机场订阅：跳过"; fi
...
print_core_switch_plan() {
	...
	say "  ${Y}－${N} Nikki、GeoX、Zashboard、机场订阅和 rule-set：保持不变"
}
...
print_component_only_plan() {
	case "$COMPONENT_ONLY_KIND" in
		...
		subscription) pcop_label="已有机场订阅与 YAML proxy-provider" ;;
...
run_component_only_workflow() {
	...
	case "$COMPONENT_ONLY_KIND" in
		...
		geox|zashboard)
			...
			;;
		subscription)
			SUBSCRIPTION_UPDATE_STATUS="not_selected"
			PROXY_PROVIDER_UPDATE_STATUS="not_selected"
			flow_title "仅机场订阅维护"
			if ! ensure_update_tools || ! begin_transaction; then
				cleanup_transaction_backups
				SUBSCRIPTION_UPDATE_STATUS="skipped"; PROXY_PROVIDER_UPDATE_STATUS="skipped"
				err "无法建立机场订阅维护环境，现有订阅保持不变"
				print_summary
				return 0
			fi
			initialize_maintenance_state
			if update_existing_subscriptions; then
				case "$SUBSCRIPTION_UPDATE_STATUS" in updated|partial) MAINT_SUCCESS=1 ;; esac
			else warn "Nikki 机场订阅更新失败，继续检查 YAML proxy-provider"; fi
			if [ "$MAINT_SUCCESS" -gt 0 ]; then
				if ! commit_transaction; then
					SUBSCRIPTION_UPDATE_STATUS="skipped"; PROXY_PROVIDER_UPDATE_STATUS="skipped"
					rollback_transaction
					warn "订阅更新后 Nikki 服务验证失败，已恢复原订阅"
					print_summary
					return 0
				fi
			else TRANSACTION_ACTIVE=0; cleanup_transaction_backups; fi
			if update_existing_proxy_providers; then :; else warn "YAML 机场 proxy-provider 更新失败或无法连接，已保留原缓存"; fi
			progress_done "机场订阅单项维护完成，正在直接进入当前维护结果"
			;;
		ruleset)
	...
	while :; do
		...
		if [ "$MAIN_CHOICE" = auto ]; then
			...
			case "$AUTO_CHOICE" in
				...
				only-subscription) configure_component_only_plan subscription || { warn "无法建立机场订阅单项维护计划"; continue; } ;;
