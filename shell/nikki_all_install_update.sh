...
#
# Nikki 全方位一键维护脚本
# 适用：OpenWrt 24.10 / 25.12 / 官方支持的 SNAPSHOT，BusyBox ash
...
usage() {
	cat <<'EOF'
用法：nikki_all_install_update.sh [选项]
...
detect_environment() {
	[ -r /etc/openwrt_release ] || fatal "未检测到 /etc/openwrt_release，本脚本仅支持 OpenWrt"
	# OpenWrt 系统自身维护的可信文件。
...
check_requirements() {
	...
	if [ "$cr_bad" -ne 0 ]; then
		say ""
		say "固件版本：OpenWrt ${OPENWRT_VERSION}（要求 >= 24.10 且官方源支持） ${cr_ow}"
	...
	if [ "$OFFICIAL_BRANCH" = "SNAPSHOT" ]; then
		say "固件版本：OpenWrt ${OPENWRT_VERSION}（Nikki 官方支持的 SNAPSHOT） ✓"
	else
		say "固件版本：OpenWrt ${OPENWRT_VERSION} >= 24.10 ✓"
	fi
...
install_or_update_nikki() {
	...
	if feed_present && feed_current; then
		info "已检测到 Nikki 官方软件源，直接更新索引和软件包"
		...
		if fetch_url "$feed_url" "$feed_script" script && grep -q 'nikkinikki.pages.dev' "$feed_script" && (cd "$WORK_DIR" && ash "$feed_script") && install_packages_from_feed; then
			ok "方案 A 添加软件源并安装完成"
			io_success=1
		else
	...
	if [ "$io_success" -ne 1 ]; then
		...
		if ! fetch_url "$install_url" "$install_script" script || ! grep -q "Nikki's installer" "$install_script" || ! (cd "$WORK_DIR" && ash "$install_script"); then
			restore_install_state
...
main() {
	...
	if [ "$ACTION" = "smart" ] && [ "$LGBM_SET" -eq 0 ] && [ -t 0 ]; then choose_lgbm; fi
