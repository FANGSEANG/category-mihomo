...
#
# 非交互示例：
#   sh nikki_all_install_update_v3.0.0.sh --action smart --lgbm auto --yes
#   sh nikki_all_install_update_v3.0.0.sh --action alpha --yes
#   sh nikki_all_install_update_v3.0.0.sh --action stable --yes
#   sh nikki_all_install_update_v3.0.0.sh --action uninstall --yes
...
usage() {
	cat <<'EOF'
用法：nikki_all_install_update_v3.0.0.sh [选项]
...
inspect_nikki_update() {
	...
	if ! feed_present || ! feed_current; then
		warn "Nikki 官方软件源缺失或与当前系统不匹配，将通过官方 Release 查询最新版本；选择继续后会自动修复软件源"
		inspect_nikki_release_fallback || warn "官方 Release 查询失败，暂时无法判断 Nikki 是否有更新"
		return 0
	fi
	info "正在刷新软件源并检查 Nikki 最新版本……"
	if ! pkg_update; then
...
manual_status_scan() {
	...
	MS_SMART_JSON="$WORK_DIR/core-smart-release.json"
	MS_ALPHA_JSON="$WORK_DIR/core-alpha-release.json"
	MS_STABLE_JSON="$WORK_DIR/core-stable-release.json"
	MS_MODEL_JSON="$WORK_DIR/model-release.json"
	MS_GEOX_JSON="$WORK_DIR/geox-release.json"
	MS_ZASH_JSON="$WORK_DIR/zashboard-release.json"
...
manual_geox_line() {
	...
	if [ ! -s "$RUN_DIR/$mgl_local" ]; then
		printf '%s：未安装；最新 %s\n' "$mgl_label" "$MS_GEOX_DATE"
	elif manual_checksum_matches "$RUN_DIR/$mgl_local" "$mgl_record"; then
		printf '%s：已安装，当前 %s，已最新\n' "$mgl_label" "$(file_date "$RUN_DIR/$mgl_local")"
	else
		printf '%s：已安装，当前 %s，可更新至 %s\n' "$mgl_label" "$(file_date "$RUN_DIR/$mgl_local")" "$MS_GEOX_DATE"
	fi
}

manual_model_status() {
	if [ ! -s "$MODEL_PATH" ]; then printf '%s\n' 'LightGBM 未安装'; return 0; fi
	...
	if [ -n "$mms_record" ] && manual_checksum_matches "$MODEL_PATH" "$mms_record"; then
		printf 'LightGBM 已安装，当前 %s，已最新\n' "$mms_label"
	else
		printf 'LightGBM 已安装，当前 %s，可更新或重新校验\n' "$mms_label"
	fi
}

manual_batch_render() {
	say ""
	menu_line "================ 手动维护：一次性多选 ================"
	if [ "$MB_INSTALLED" -eq 1 ]; then
		mbr_nv="$(nikki_version 2>/dev/null || true)"; [ -n "$mbr_nv" ] || mbr_nv="版本未知"
		case "$NIKKI_UPDATE_STATE" in latest) mbr_ns="已最新" ;; update) mbr_ns="可更新至 ${NIKKI_AVAILABLE_VERSION}" ;; *) mbr_ns="最新状态查询失败" ;; esac
		say "${B}${G}Nikki：已安装，当前 ${mbr_nv}，${mbr_ns}${N}"
	else
		say "${B}${R}Nikki：未安装（本次必须选择 1）${N}"
	fi
	menu_line "  [ ] 1）安装、更新或修复 Nikki 插件主体"
	say ""
	if core_is_installed; then
		mbr_cv="$(core_installed_version)"; mbr_ck="$(core_kind_label "$(core_installed_kind "$mbr_cv")")"
		say "${B}${G}内核：已安装 ${mbr_ck}内核，当前版本 ${mbr_cv}${N}"
	else
		say "${B}${R}内核：未安装（本次必须在 2-4 中三选一）${N}"
	fi
	menu_line "  [ ] 2）Smart 内核；最新 $(manual_record_name "$MS_SMART_RECORD")"
	menu_line "  [ ] 3）Dev 开发预览版内核；最新 $(manual_record_name "$MS_ALPHA_RECORD")"
	menu_line "  [ ] 4）稳定版内核；最新 $(manual_record_name "$MS_STABLE_RECORD")"
	say ""
	say "${B}${Y}$(manual_model_status)；仅选择 2）Smart 内核时可选${N}"
	menu_line "  [ ] 5）自动选择合适的 LightGBM 模型"
	menu_line "  [ ] 6）大型模型（Model-large.bin），约 $(manual_size_label "$MS_MODEL_LARGE_RECORD" '20.4 MiB')"
	menu_line "  [ ] 7）中型模型（Model-middle.bin），约 $(manual_size_label "$MS_MODEL_MIDDLE_RECORD" '10.6 MiB')"
	menu_line "  [ ] 8）轻量模型（Model.bin），约 $(manual_size_label "$MS_MODEL_SMALL_RECORD" '7.47 MiB')"
	say ""
	say "${B}${Y}GeoX 数据库状态：${N}"
	say "  $(manual_geox_line geosite GeoSite.dat "$MS_GEOSITE_RECORD")"
	say "  $(manual_geox_line geoip GeoIP.dat "$MS_GEOIP_RECORD")"
	say "  $(manual_geox_line mmdb Country.mmdb "$MS_MMDB_RECORD")"
	say "  $(manual_geox_line asn ASN.mmdb "$MS_ASN_RECORD")"
	menu_line "  [ ] 9）更新整套 GeoX 数据库"
	say ""
	if zashboard_is_installed; then
		mbr_zv="$(zashboard_installed_version)"
		case "$mbr_zv" in "$MS_ZASH_TAG"*) mbr_zs="已最新" ;; *) mbr_zs="可更新至 ${MS_ZASH_TAG}" ;; esac
		say "${B}${G}Zashboard：已安装，当前 ${mbr_zv}，${mbr_zs}${N}"
	else
		say "${B}${Y}Zashboard：未安装；最新 ${MS_ZASH_TAG}${N}"
	fi
	menu_line "  [ ] 10）完整版（dist.zip，全部内置字体），约 $(manual_size_label "$MS_ZASH_FULL_RECORD" '未知')"
	menu_line "  [ ] 11）无字体版（dist-no-fonts.zip），约 $(manual_size_label "$MS_ZASH_NOFONTS_RECORD" '未知')"
	menu_line "  [ ] 12）CDN 字体版（dist-cdn-fonts.zip），约 $(manual_size_label "$MS_ZASH_CDN_RECORD" '未知')"
	menu_line "  [ ] 13）FiraSans 字体版（dist-firasans-only.zip），约 $(manual_size_label "$MS_ZASH_FIRA_RECORD" '未知')"
	menu_line "  [ ] 14）MiSans 字体版（dist-misans-only.zip），约 $(manual_size_label "$MS_ZASH_MI_RECORD" '未知')"
	menu_line "  [ ] 15）PingFang 字体版（dist-pingfang-only.zip），约 $(manual_size_label "$MS_ZASH_PING_RECORD" '未知')"
	menu_line "  [ ] 16）Sarasa 字体版（dist-sarasa-only.zip），约 $(manual_size_label "$MS_ZASH_SARASA_RECORD" '未知')"
	menu_line "  [ ] 0）直接返回主菜单（必须单独选择）"
	menu_line "======================================================"
	say "${B}${Y}输入多个编号时用空格或英文逗号分隔，例如：1 2 5 9 10${N}"
}
...
manual_batch_reset() {
	...
	MB_ZASH_FIRA=0; MB_ZASH_MI=0; MB_ZASH_PING=0; MB_ZASH_SARASA=0; MB_RETURN=0
}
...
manual_batch_parse() {
	...
	mbp_action_count=$((MB_NIKKI + mbp_core_count + mbp_model_count + MB_GEOX + mbp_zash_count))
	...
	if [ "$MB_INSTALLED" -ne 1 ] && [ "$MB_NIKKI" -ne 1 ]; then warn "Nikki 未安装，本次必须选择 1）Nikki 插件主体"; return 1; fi
	if ! core_is_installed && [ "$mbp_core_count" -eq 0 ]; then warn "内核未安装，本次必须在 2-4 中三选一"; return 1; fi
	if [ "$mbp_model_count" -gt 0 ] && [ "$MB_CORE_SMART" -ne 1 ]; then warn "LightGBM 仅能与 2）Smart 内核同时选择"; return 1; fi
...
manual_batch_apply() {
	...
	ZASH_CHOICE=update
	if [ "$MB_ZASH_FULL" -eq 1 ]; then ZASH_ASSET=dist.zip; ZASH_VARIANT_LABEL=full
	elif [ "$MB_ZASH_NOFONTS" -eq 1 ]; then ZASH_ASSET=dist-no-fonts.zip; ZASH_VARIANT_LABEL=no-fonts
	elif [ "$MB_ZASH_CDN" -eq 1 ]; then ZASH_ASSET=dist-cdn-fonts.zip; ZASH_VARIANT_LABEL=cdn-fonts
	elif [ "$MB_ZASH_FIRA" -eq 1 ]; then ZASH_ASSET=dist-firasans-only.zip; ZASH_VARIANT_LABEL=FiraSans
	elif [ "$MB_ZASH_MI" -eq 1 ]; then ZASH_ASSET=dist-misans-only.zip; ZASH_VARIANT_LABEL=MiSans
	elif [ "$MB_ZASH_PING" -eq 1 ]; then ZASH_ASSET=dist-pingfang-only.zip; ZASH_VARIANT_LABEL=PingFang
	elif [ "$MB_ZASH_SARASA" -eq 1 ]; then ZASH_ASSET=dist-sarasa-only.zip; ZASH_VARIANT_LABEL=Sarasa
...
manual_batch_menu() {
	...
	while :; do
		...
		read_user_input || fatal "无法读取手动维护选择"
...
uninstall_nikki() {
	...
	# 不以命令退出码代替结果：逐项确认软件包、数据目录和软件源确实已经移除。
	un_residual=0
	for un_pkg in $un_langs luci-app-nikki nikki mihomo-meta mihomo-alpha; do
	...
	case "$PKG_MANAGER" in
		opkg) if grep -q nikki /etc/opkg/customfeeds.conf 2>/dev/null; then warn "OPKG 软件源中仍存在 Nikki 条目"; un_residual=1; fi ;;
		apk)
			if grep -q nikki /etc/apk/repositories.d/customfeeds.list 2>/dev/null; then warn "APK 软件源中仍存在 Nikki 条目"; un_residual=1; fi
			if [ -e /etc/apk/keys/nikki.pem ]; then warn "APK 信任密钥仍存在：/etc/apk/keys/nikki.pem"; un_residual=1; fi
			;;
	esac
...
main() {
	...
	while :; do
		...
		if [ "$MAIN_CHOICE" = auto ]; then
			...
			info "前置环境检查已通过；将自动执行 Nikki 安装或更新、组件维护及最终结果汇总，无需再次人工选择"
		else
