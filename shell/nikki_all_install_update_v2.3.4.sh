...
#
# 非交互示例：
#   sh nikki_all_install_update_v2.3.4.sh --action smart --lgbm auto --yes
#   sh nikki_all_install_update_v2.3.4.sh --action alpha --yes
#   sh nikki_all_install_update_v2.3.4.sh --action stable --yes
#   sh nikki_all_install_update_v2.3.4.sh --action uninstall --yes
...
usage() {
	cat <<'EOF'
用法：nikki_all_install_update_v2.3.4.sh [选项]
...
prepare_zashboard() {
	...
	# 使用包含全部内置字体的完整版；同时与 Nikki 中配置的官方 latest URL 保持一致。
	pz_asset="dist.zip"
	...
	if command -v uci >/dev/null 2>&1; then
		...
		case "$pz_current_url" in
			*Zephyruso/zashboard*/dist.zip) pz_keep_url="$pz_current_url" ;;
			*) case "$pz_yaml_url" in *Zephyruso/zashboard*/dist.zip) pz_keep_url="$pz_yaml_url" ;; *) pz_keep_url='https://github.com/Zephyruso/zashboard/releases/latest/download/dist.zip' ;; esac ;;
		esac
		info "设置 Zashboard 完整版下载地址：$pz_keep_url"
...
run_maintenance() {
	...
	CORE_KIND_NEW="$(state_get CORE_KIND)"
	NEW_CORE_VERSION="$(state_get CORE_VERSION)"
	MODEL_LABEL_NEW="$(state_get MODEL)"
	GEOX_DATE_NEW="$(state_get GEOX_DATE)"
	ZASH_VERSION_NEW="$(state_get ZASH_VERSION)"
	if [ -z "$NEW_CORE_VERSION" ] && [ -x "$CORE_PATH" ]; then NEW_CORE_VERSION="$("$CORE_PATH" -v 2>/dev/null | head -n 1 || true)"; fi
	if [ -z "$CORE_KIND_NEW" ]; then
...
print_summary() {
	...
	case "$CORE_UPDATE_STATUS" in updated) ps_core_result="》》》已最新" ;; skipped) ps_core_result="》》》本次更新失败，已保留原版本" ;; *) ps_core_result="" ;; esac
	case "$MODEL_UPDATE_STATUS" in updated) ps_model_result="》》》已最新" ;; skipped) ps_model_result="》》》本次更新失败，已保留原版本" ;; *) ps_model_result="" ;; esac
	case "$GEOX_UPDATE_STATUS" in updated) ps_geox_result="》》》已最新" ;; skipped) ps_geox_result="》》》本次更新失败，已保留原版本" ;; *) ps_geox_result="" ;; esac
	case "$ZASH_UPDATE_STATUS" in updated) ps_zash_result="》》》已最新" ;; skipped) ps_zash_result="》》》本次更新失败，已保留原版本" ;; *) ps_zash_result="" ;; esac
	case "$ps_kind" in
		smart) say "当前 Smart 内核：${ps_core} ${ps_core_result}"; say "      LGBM模型：${ps_model:-unknown} ${ps_model_result}" ;;
		alpha) say "当前开发预览版内核（Prerelease-Alpha）：${ps_core} ${ps_core_result}" ;;
		stable) say "当前稳定版内核（meta）：${ps_core} ${ps_core_result}" ;;
		*) say "当前内核：${ps_core}" ;;
	esac
	say ""
	say "当前 GeoX数据库："
	for ps_line in "geosite:GeoSite.dat" "geoip:GeoIP.dat" "mmdb:Country.mmdb" "asn:ASN.mmdb"; do
		...
		if [ -s "$RUN_DIR/$ps_file" ]; then say "${ps_label}: ${ps_geox} ${ps_geox_result}"; else say "${ps_label}: 未安装"; fi
	done
	say ""
	if [ -s "$UI_TARGET/index.html" ]; then say "当前 zashboard：${ps_zash} ${ps_zash_result}"; else say "当前 zashboard：未安装"; fi
	say "================================================"
}
...
startup_menu() {
	while :; do
		say ""
		menu_line "================ Nikki 主菜单 ================"
		say "检测结果：Nikki 已安装（当前版本：$(nikki_version 2>/dev/null || printf '%s' unknown)）"
		menu_line "  1）继续更新或修复 Nikki 及其依赖、内核、GeoX 和 Zashboard"
		menu_line "  2）一键卸载重置并清理残留（卸载后可返回重新安装）"
		menu_line "  3）直接退出脚本"
		menu_line "================================================"
		prompt '>>> 请手动选择 [1-3]：'
		read_user_input || fatal "无法读取主菜单选项"
		case "$USER_INPUT" in 1) START_CHOICE="update"; return 0 ;; 2) START_CHOICE="uninstall"; return 0 ;; 3) START_CHOICE="exit"; return 0 ;; *) warn "无效选项，请重新输入" ;; esac
	done
}
...
choose_action() {
	while :; do
		say ""
		menu_line "================ Nikki 全方位维护菜单 ================"
		menu_line "  1）Smart 内核（Alpha with Smart Group）+ LGBM + GeoX + Zashboard"
		menu_line "  2）开发预览版内核（Prerelease-Alpha）+ GeoX + Zashboard"
		menu_line "  3）稳定版内核（meta）+ GeoX + Zashboard"
		menu_line "  0）返回上一级菜单"
		menu_line "======================================================="
		prompt '>>> 请手动选择 [0-3]：'
...
run_update_workflow() {
	...
	while :; do
		...
		if [ "$ACTION" = "smart" ] && [ "$LGBM_SET" -eq 0 ] && [ "$ASSUME_YES" -eq 0 ]; then
			# 命令行模式不进入交互菜单；未指定模型时沿用默认 auto。
...
main() {
	...
	while :; do
		...
		if pkg_is_installed nikki; then
			startup_menu || fatal "无法读取主菜单选项"
		else
			info "未检测到 Nikki，将继续进行系统环境检测并安装最新版及完整维护组件"
			START_CHOICE="update"
		fi
		[ "$START_CHOICE" != "exit" ] || exit 0

		if [ "$START_CHOICE" = "uninstall" ]; then
			detect_environment
			if command -v curl >/dev/null 2>&1 || command -v wget >/dev/null 2>&1; then select_downloader; fi
			if ! uninstall_nikki; then
				warn "卸载未执行，返回主菜单"
				continue
			fi
