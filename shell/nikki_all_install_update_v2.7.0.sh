...
#
# 非交互示例：
#   sh nikki_all_install_update_v2.7.0.sh --action smart --lgbm auto --yes
#   sh nikki_all_install_update_v2.7.0.sh --action alpha --yes
#   sh nikki_all_install_update_v2.7.0.sh --action stable --yes
#   sh nikki_all_install_update_v2.7.0.sh --action uninstall --yes
...
usage() {
	cat <<'EOF'
用法：nikki_all_install_update_v2.7.0.sh [选项]
...
confirm_continue_prompt() {
	ccp_message="$1"
	ccp_yes_label="$2"
	ccp_no_label="${3:-返回当前维护项目菜单}"
	while :; do
		say "${B}${Y}${ccp_message}${N}"
		menu_line "  1）是，${ccp_yes_label}"
		menu_line "  2）否，${ccp_no_label}"
		prompt '>>> 请手动选择 [1-2]：'
		read_user_input || fatal "无法读取确认选项"
		case "$USER_INPUT" in 1) return 0 ;; 2) return 1 ;; *) warn "无效选项，请输入 1 或 2" ;; esac
	done
}
...
confirm_core_selection() {
	ccs_target="$(core_kind_label "$ACTION")"
	if ! core_is_installed; then
		info "未检测到 Mihomo 内核，将直接安装所选的 ${ccs_target}内核"
		return 0
	fi
	ccs_version="$(core_installed_version)"
	ccs_current="$(core_kind_label "$(core_installed_kind "$ccs_version")")"
	say "${B}${G}系统已安装 ${ccs_current}内核（版本：${ccs_version}）${N}"
	say "${B}${C}本次选择：${ccs_target}内核${N}"
	confirm_continue_prompt "是否继续更新或切换内核？" "继续更新或切换"
}

geox_is_installed() {
	for gii_file in GeoSite.dat GeoIP.dat Country.mmdb ASN.mmdb; do [ -s "$RUN_DIR/$gii_file" ] || return 1; done
	return 0
}

geox_has_any_file() {
	for gha_file in GeoSite.dat GeoIP.dat Country.mmdb ASN.mmdb; do [ ! -s "$RUN_DIR/$gha_file" ] || return 0; done
	return 1
}

geox_installed_version() {
	giv_version="$(state_get GEOX_DATE)"
	[ -n "$giv_version" ] || giv_version="$(file_date "$RUN_DIR/GeoSite.dat")"
	[ -n "$giv_version" ] || giv_version="版本未知"
	printf '%s\n' "$giv_version"
}

confirm_geox_update() {
	if geox_is_installed; then
		say "${B}${G}系统已安装完整 GeoX 数据库（版本/日期：$(geox_installed_version)）${N}"
		confirm_continue_prompt "是否继续更新 GeoX 数据库？" "继续更新"
		return $?
	fi
	if geox_has_any_file; then
		warn "检测到 GeoX 数据库文件不完整，将直接补全并更新"
	else
		info "未检测到 GeoX 数据库，将直接安装"
	fi
	return 0
}
...
confirm_zashboard_update() {
	if ! zashboard_is_installed; then
		info "未检测到 Zashboard，将直接安装所选版本"
		return 0
	fi
	say "${B}${G}系统已安装 Zashboard（当前版本：$(zashboard_installed_version)）${N}"
	say "${B}${C}本次选择：${ZASH_VARIANT_LABEL}（${ZASH_ASSET}）${N}"
	confirm_continue_prompt "是否继续更新或切换 Zashboard 版本？" "继续更新或切换"
}
...
manual_nikki_menu() {
	mnm_installed="$1"
	if [ "$mnm_installed" -ne 1 ]; then
		say ""
		say "${B}${Y}系统未安装 Nikki，将直接进入 Nikki 安装流程。${N}"
		MANUAL_NIKKI_CHOICE="update"
		return 0
	fi
	mnm_version="$(nikki_version 2>/dev/null || true)"
	[ -n "$mnm_version" ] || mnm_version="版本未知"
	inspect_nikki_update
	case "$NIKKI_UPDATE_STATE" in
		latest) say "${B}${G}系统已安装 Nikki（当前版本：${mnm_version}，已经最新）${N}" ;;
		update) say "${B}${Y}系统已安装 Nikki（当前版本：${mnm_version}，有更新版本：${NIKKI_AVAILABLE_VERSION}）${N}" ;;
		*) say "${B}${Y}系统已安装 Nikki（当前版本：${mnm_version}，最新版本状态暂时无法确认）${N}" ;;
	esac
	if confirm_continue_prompt "是否继续安装、更新或修复 Nikki？" "进入 Nikki 安装/更新流程" "显示跳过 Nikki 或返回上一级选项"; then
		MANUAL_NIKKI_CHOICE="update"
		return 0
	fi
	while :; do
		say ""
		menu_line "================ Nikki 后续操作 ================"
		menu_line "  1）跳过 Nikki 安装更新，并进入内核、GeoX、Zashboard 维护"
		menu_line "  2）返回上一级菜单"
		menu_line "==============================================="
		prompt '>>> 请手动选择 [1-2]：'
		read_user_input || fatal "无法读取手动维护菜单选项"
		case "$USER_INPUT" in
			1) MANUAL_NIKKI_CHOICE="skip"; return 0 ;;
			2) MANUAL_NIKKI_CHOICE="return"; return 0 ;;
			*) warn "无效选项，请重新输入" ;;
		esac
	done
}
...
choose_action() {
	while :; do
		...
		case "$ca_choice" in
			1) ACTION=smart; if confirm_core_selection; then return 0; fi; ACTION="" ;;
			2) ACTION=alpha; if confirm_core_selection; then return 0; fi; ACTION="" ;;
			3) ACTION=stable; if confirm_core_selection; then return 0; fi; ACTION="" ;;
...
choose_geox() {
	while :; do
		...
		case "$USER_INPUT" in
			1) GEOX_CHOICE=update; if confirm_geox_update; then return 0; fi; GEOX_CHOICE="" ;;
			2) GEOX_CHOICE=skip; return 0 ;;
			0) return 2 ;;
			*) warn "无效 GeoX 选项，请重新输入" ;;
		esac
	done
}
...
choose_zashboard() {
	while :; do
		...
		case "$USER_INPUT" in
			""|1) ZASH_CHOICE=update; ZASH_ASSET=dist.zip; ZASH_VARIANT_LABEL=full ;;
			2) ZASH_CHOICE=update; ZASH_ASSET=dist-no-fonts.zip; ZASH_VARIANT_LABEL=no-fonts ;;
			3) ZASH_CHOICE=update; ZASH_ASSET=dist-cdn-fonts.zip; ZASH_VARIANT_LABEL=cdn-fonts ;;
			4) ZASH_CHOICE=update; ZASH_ASSET=dist-firasans-only.zip; ZASH_VARIANT_LABEL=FiraSans ;;
			5) ZASH_CHOICE=update; ZASH_ASSET=dist-misans-only.zip; ZASH_VARIANT_LABEL=MiSans ;;
			6) ZASH_CHOICE=update; ZASH_ASSET=dist-pingfang-only.zip; ZASH_VARIANT_LABEL=PingFang ;;
			7) ZASH_CHOICE=update; ZASH_ASSET=dist-sarasa-only.zip; ZASH_VARIANT_LABEL=Sarasa ;;
			8) ZASH_CHOICE=skip; return 0 ;;
			0) return 2 ;;
			*) warn "无效 Zashboard 选项，请重新输入"; continue ;;
		esac
		if confirm_zashboard_update; then return 0; fi
		ZASH_CHOICE=""
	done
}
